...
import delete.diagram.part.DeleteDiagramEditor;
 ...
 * @generated
 */
public class DeleteValidationDecoratorProvider extends AbstractProvider implements IDecoratorProvider {
	...
	* @generated
	*/
	private static final String MARKER_TYPE = DeleteDiagramEditorPlugin.ID + ".diagnostic"; //$NON-NLS-1$
	...
	*/
	public void createDecorators(IDecoratorTarget decoratorTarget) {
		...
		if (editPart instanceof GraphicalEditPart || editPart instanceof AbstractConnectionEditPart) {
			...
			if (((DiagramEditDomain) ed).getEditorPart() instanceof DeleteDiagramEditor) {
	...
	*/
	public boolean provides(IOperation operation) {
		...
		return view != null && DeleteSurfaceEditPart.MODEL_ID.equals(DeleteVisualIDRegistry.getModelID(view));
	}
		...
		PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				try {
					...
					DeleteDiagramEditorPlugin.getInstance().logError("Decorator refresh failure", e); //$NON-NLS-1$
				}
			}
		});
	}
		...
		*/
		public StatusDecorator(IDecoratorTarget decoratorTarget) {
			...
			try {
				...
				DeleteDiagramEditorPlugin.getInstance().logError("ViewID access failure", e); //$NON-NLS-1$			
			}
		}
		...
		*/
		public void refresh() {
			...
			try {
				...
				DeleteDiagramEditorPlugin.getInstance().logError("Validation markers refresh failure", e); //$NON-NLS-1$
			}
		...
		*/
		private String getType(IMarker marker) {
			try {
				...
				DeleteDiagramEditorPlugin.getInstance().logError("Validation marker refresh failure", e); //$NON-NLS-1$
				return ""; //$NON-NLS-1$
			}
		}
	}
