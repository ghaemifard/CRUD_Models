 ...
 * @generated
 */
public class CallSurfaceQuery2ItemSemanticEditPolicy extends InsertBaseItemSemanticEditPolicy {

	/**
	* @generated
	*/
	public CallSurfaceQuery2ItemSemanticEditPolicy() {
	...
	*/
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (InsertVisualIDRegistry.getVisualID(node)) {
			case CallSurfaceQueryCallSurfaceQueryParamsCompartment2EditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (InsertVisualIDRegistry.getVisualID(cnode)) {
