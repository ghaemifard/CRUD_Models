...
import insert.diagram.part.InsertDiagramEditor;
 ...
 * @generated
 */
public class InsertValidationDecoratorProvider extends AbstractProvider implements IDecoratorProvider {
	...
	* @generated
	*/
	private static final String MARKER_TYPE = InsertDiagramEditorPlugin.ID + ".diagnostic"; //$NON-NLS-1$
	...
	*/
	public void createDecorators(IDecoratorTarget decoratorTarget) {
		...
		if (editPart instanceof GraphicalEditPart || editPart instanceof AbstractConnectionEditPart) {
			...
			if (((DiagramEditDomain) ed).getEditorPart() instanceof InsertDiagramEditor) {
	...
	*/
	public boolean provides(IOperation operation) {
		...
		return view != null && InsertSurfaceEditPart.MODEL_ID.equals(InsertVisualIDRegistry.getModelID(view));
	}
		...
		PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				try {
					...
					InsertDiagramEditorPlugin.getInstance().logError("Decorator refresh failure", e); //$NON-NLS-1$
				}
			}
		});
	}
		...
		*/
		public StatusDecorator(IDecoratorTarget decoratorTarget) {
			...
			try {
				...
				InsertDiagramEditorPlugin.getInstance().logError("ViewID access failure", e); //$NON-NLS-1$			
			}
		}
		...
		*/
		public void refresh() {
			...
			try {
				...
				InsertDiagramEditorPlugin.getInstance().logError("Validation markers refresh failure", e); //$NON-NLS-1$
			}
		...
		*/
		private String getType(IMarker marker) {
			try {
				...
				InsertDiagramEditorPlugin.getInstance().logError("Validation marker refresh failure", e); //$NON-NLS-1$
				return ""; //$NON-NLS-1$
			}
		}
	}
