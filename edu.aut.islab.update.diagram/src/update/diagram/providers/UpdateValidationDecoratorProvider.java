...
import update.diagram.part.UpdateDiagramEditor;
 ...
 * @generated
 */
public class UpdateValidationDecoratorProvider extends AbstractProvider implements IDecoratorProvider {
	...
	* @generated
	*/
	private static final String MARKER_TYPE = UpdateDiagramEditorPlugin.ID + ".diagnostic"; //$NON-NLS-1$
	...
	*/
	public void createDecorators(IDecoratorTarget decoratorTarget) {
		...
		if (editPart instanceof GraphicalEditPart || editPart instanceof AbstractConnectionEditPart) {
			...
			if (((DiagramEditDomain) ed).getEditorPart() instanceof UpdateDiagramEditor) {
	...
	*/
	public boolean provides(IOperation operation) {
		...
		return view != null && UpdateSurfaceEditPart.MODEL_ID.equals(UpdateVisualIDRegistry.getModelID(view));
	}
		...
		PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				try {
					...
					UpdateDiagramEditorPlugin.getInstance().logError("Decorator refresh failure", e); //$NON-NLS-1$
				}
			}
		});
	}
		...
		*/
		public StatusDecorator(IDecoratorTarget decoratorTarget) {
			...
			try {
				...
				UpdateDiagramEditorPlugin.getInstance().logError("ViewID access failure", e); //$NON-NLS-1$			
			}
		}
		...
		*/
		public void refresh() {
			...
			try {
				...
				UpdateDiagramEditorPlugin.getInstance().logError("Validation markers refresh failure", e); //$NON-NLS-1$
			}
		...
		*/
		private String getType(IMarker marker) {
			try {
				...
				UpdateDiagramEditorPlugin.getInstance().logError("Validation marker refresh failure", e); //$NON-NLS-1$
				return ""; //$NON-NLS-1$
			}
		}
	}
