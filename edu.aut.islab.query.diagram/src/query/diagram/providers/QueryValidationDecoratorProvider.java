...
import query.diagram.part.QueryDiagramEditor;
 ...
 * @generated
 */
public class QueryValidationDecoratorProvider extends AbstractProvider implements IDecoratorProvider {
	...
	* @generated
	*/
	private static final String MARKER_TYPE = QueryDiagramEditorPlugin.ID + ".diagnostic"; //$NON-NLS-1$
	...
	*/
	public void createDecorators(IDecoratorTarget decoratorTarget) {
		...
		if (editPart instanceof GraphicalEditPart || editPart instanceof AbstractConnectionEditPart) {
			...
			if (((DiagramEditDomain) ed).getEditorPart() instanceof QueryDiagramEditor) {
	...
	*/
	public boolean provides(IOperation operation) {
		...
		return view != null && QuerySurfaceEditPart.MODEL_ID.equals(QueryVisualIDRegistry.getModelID(view));
	}
		...
		PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				try {
					...
					QueryDiagramEditorPlugin.getInstance().logError("Decorator refresh failure", e); //$NON-NLS-1$
				}
			}
		});
	}
		...
		*/
		public StatusDecorator(IDecoratorTarget decoratorTarget) {
			...
			try {
				...
				QueryDiagramEditorPlugin.getInstance().logError("ViewID access failure", e); //$NON-NLS-1$			
			}
		}
		...
		*/
		public void refresh() {
			...
			try {
				...
				QueryDiagramEditorPlugin.getInstance().logError("Validation markers refresh failure", e); //$NON-NLS-1$
			}
		...
		*/
		private String getType(IMarker marker) {
			try {
				...
				QueryDiagramEditorPlugin.getInstance().logError("Validation marker refresh failure", e); //$NON-NLS-1$
				return ""; //$NON-NLS-1$
			}
		}
	}
